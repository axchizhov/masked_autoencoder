...
if __name__ == '__main__':
    ...
    writer = SummaryWriter(f'logs/mae_pretrain {timestamp}')
    # writer.add_hparams(vars(args), {})
    
    setup_seed(args.seed)

    batch_size = args.batch_size
    # To limit device memory overflow
    ...
    for e in tqdm(range(args.total_epoch)):
        model.train()
        losses = []
        for img, label in tqdm(iter(dataloader), leave=False):
            step_count += 1
            img = img.to(device)
            ...
            writer.add_scalar('mae_batch_loss', loss.item(), global_step=step_count)
        lr_scheduler.step()
